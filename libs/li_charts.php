...
class Li_charts {
	...
	public function parse_stats($sites,$period,$cat) {
		...
		$rc_csv = new RollingCurl(array($this,"get_csv_stats"));
		...
		if($cat=="index") {
			$rc_today = new RollingCurl(array($this,"get_today_stat"));
		...
		foreach ($sites as $site) {
			...
			#Формируем урл
			$get_url = "http://www.liveinternet.ru/stat/$url_stat/$cat.csv?{$ids}graph=csv$liperiod";
	...
	public function auth_li($domain,$pass) {
		...
		$c = curl_init("https://www.liveinternet.ru/stat/");
